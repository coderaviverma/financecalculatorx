export default {
  ...
  sections: {
    ...
    explanationHtml: `
      ...
      <p>So why does snowball exist? Because it front-loads wins. Killing the $5,000 personal loan first (snowball's order) produces a paid-off account months earlier than avalanche's order does, and each cleared debt frees a minimum payment that makes the next one fall faster, a visible feedback loop that can make a plan easier to maintain. Behavioral-finance literature on debt repayment suggests that sticking with a plan matters more than optimizing it: the best strategy is worth nothing abandoned in month six.</p>
      <p>The honest takeaway from running real numbers: for many debt profiles the strategies differ by tens of dollars per month of effort, not thousands. When the gap is small, the financial cost of choosing either strategy is limited; the calculator quantifies that difference, but it cannot predict which plan a person will maintain. When one debt's rate towers over the rest, as payday loans and many credit cards do, avalanche's modeled advantage grows substantially. This calculator's job is to show you the actual size of the gap for <em>your</em> debts, so the choice is informed rather than ideological.</p>`,
    formulaHtml: `
