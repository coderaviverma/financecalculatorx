export default {
  ...
  sections: {
    howToHtml: `
      <p>Start with the home price and your planned down payment — the difference is the loan amount. Add the interest rate you've been quoted and the term (30 years is the default; try 15 to see the trade-off). Then refine the estimate with the costs lenders add to your escrow:</p>
      <ul>
    ...
    factorsHtml: `
      <ul>
        ...
        <li><strong>Down payment.</strong> More down means a smaller loan, a lower payment, and — at 20% — no PMI. Below 20%, PMI adds a recurring cost that buys you nothing as a borrower.</li>
  ...
  faq: [
    ...
    },
    {
      ...
      aHtml: `<p>The amortization math is universal for fixed-rate repayment mortgages, and you can switch the display currency in the header. PMI is a US concept — in other markets the equivalent is a lender's insurance premium or simply a higher rate above certain LTVs — so set the PMI field to 0 and add any local recurring charges under HOA/society fees.</p>`,
    },
  ],
};
