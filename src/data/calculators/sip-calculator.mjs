export default {
  ...
  sections: {
    howToHtml: `
      <p>Enter your monthly SIP amount, the annual return you want to assume, and how long you'll stay invested. Two optional fields change the picture meaningfully:</p>
      <ul>
    ...
    explanationHtml: `
      ...
      <p>A SIP — systematic investment plan — is a standing instruction to buy mutual fund units for a fixed amount on a fixed date each month. The amount is constant; the number of units varies with that day's NAV. When the market falls your ₹10,000 buys more units, when it rises it buys fewer, so your average purchase cost per unit sits below the average NAV over the period. That's rupee-cost averaging, and it's worth stating honestly: it is not a return-enhancing trick — a rising market means your early cheap units were the best buys you'll get, and a lump sum invested at the start would have caught them all. The real benefit of a SIP is <strong>discipline</strong>: it automates investing before spending can happen, removes the "is now a good time?" decision entirely, and keeps you buying through downturns, which is precisely when most investors stop.</p>
    ...
    formulaHtml: `
      ...
      <p>With an annual step-up of <code>s</code>, each year's installments are (1 + s) times the previous year's, so the calculator simulates month by month rather than using a single closed form — the same engine that produces the year-by-year table. An initial lump sum adds a P(1 + i)<sup>n</sup> term on top.</p>
    ...
    exampleHtml: `
      <div class="example-block"><div class="ex-head">Example: ₹10,000/month for 15 years at 12%, with and without step-up</div><div class="ex-body">
        <p><strong>Flat SIP:</strong> 180 installments totalling ₹18,00,000 grow to <strong>₹49,95,802</strong> — gains of ₹31,95,802.</p>
    ...
    factorsHtml: `
      <ul>
        <li><strong>Duration beats everything.</strong> The flat default SIP reaches ₹23 lakh in 10 years, ₹49.96 lakh in 15, ₹98.93 lakh in 20. Each extra five years roughly doubles the outcome at 12% — stopping early is the most expensive choice available.</li>
  ...
  faq: [
    ...
    },
    {
      ...
      aHtml: `<p>Falling markets are when your fixed installment buys the most units — pausing then defeats the one mechanism that makes averaging useful and usually stems from treating the SIP as a market call rather than a savings habit. If cash flow genuinely tightens, reducing the amount beats stopping: the year-by-year table shows how much of the final value each year's installments carry.</p>`,
    },
  ],
};
