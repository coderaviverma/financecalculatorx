...
<h2>Where an extra payment actually goes</h2>
...
<p>An equivalent framing that makes the trade-offs obvious later: paying down a 6.5% loan is the same as investing that money at a <strong>guaranteed 6.5% return</strong> for the loan's remaining term. Guaranteed, because the interest you avoid does not depend on any market.</p>
...
<tbody>
<tr><td>Never (baseline)</td><td>$382,633</td><td>—</td><td>360 months</td></tr>
...
<p><strong>United States:</strong> prepayment penalties are rare on consumer loans and tightly restricted on most mortgages originated since 2014 (where allowed at all, they are generally limited to the first three years). Some personal and auto loans instead use precomputed interest, which blunts the benefit of prepaying — read the note before assuming.
<strong>India:</strong> the RBI has barred foreclosure charges and prepayment penalties on floating-rate loans to individuals; fixed-rate loans may still carry them.
...
<p>Since prepaying a loan is a guaranteed return equal to the loan's rate, the comparison with investing is at least honest to state: a 6.5% guaranteed, risk-free, tax-situation-dependent return versus an <em>expected</em> but volatile market return that has historically been higher over long periods — with no guarantee over your particular period. There is no universally correct answer, but there is a defensible order of operations:</p>
<ul>
...
<p>Extra payments work through one lever: principal removed today stops costing interest for every month that remains. That makes them most powerful early in a long loan, entirely dependent on being applied to principal, and equivalent to a guaranteed return at the loan's rate — which is exactly the standard to hold them to when deciding between prepaying, investing, or attacking a more expensive debt first.</p>
`,
};
